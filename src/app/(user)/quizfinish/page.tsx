...
const Page = () => {
  ...
  const [tournmentId, setTournmentId] = useState();
  ...
  const getQuizById = async () => {
    ...
    try {
      if (quizId !== null) {
        const response = await storeService.getQuizById(quizId);
        console.log(response);
        //@ts-ignore
        setQuiz(response);
        setTournmentId(response?.tournamentId);
      } else {
        console.log("quizId is null");
      }
    } catch (error) {
  ...
  const getTournmentById = async () => {
    const storeService = new StoreTournmentDataService();
    try {
      //@ts-ignore
      const response = await storeService.getQuizByTournmentId(tournmentId);
      console.log(response);
    } catch (error) {
      console.log("Error in getting all quiz api");
    }
  };
  ...
  const goToLeadrBoard = () => {
    if (quizId) {
      router.push(`/newlayout/leadrboard?quizId=${quizId}`);
    }
  };
  ...
  //@ts-ignore
  const colorOverlay = quiz?.colorSelect || "rgba(0, 0, 0, 0.5)";
      ...
      <div className="bgPurple">
        ...
        >
          ...
          <div
            ...
            style={{
              zIndex: 1,
              backgroundColor: colorOverlay || "#6D10CA",
            }}
          ></div>
          <div className="p50" style={{ position: "relative", zIndex: 2 }}>
        ...
        <div className="quizCompletedDiv text-center mb-9 relative z-10">
          <Image
            src={av3}
            alt=""
            height={100}
            width={100}
            className="mx-60 relative"
            style={{ marginTop: "-45px" }}
          />

          <div className="quizComHeadingDiv mt-3">
            <h3 className="quizComHeading">Quiz completed </h3>
          ...
          <div className="flex items-center justify-center mt-3">
            ...
            <h1 className="quizTotalPoints ps-3">{score}</h1>
          </div>
