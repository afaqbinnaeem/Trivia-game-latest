...
import { Client, Databases, ID, Query, QueryTypesList, Storage } from 'appwrite'
...
export class StoreTournmentDataService {
    ...
    bucket
    constructor() {
      this.client.setEndpoint(`${process.env.NEXT_PUBLIC_APPWRITE_ENDPOINT}`).setProject(`${process.env.NEXT_PUBLIC_PROJECTID}`)
  ...
  async updateTournment({ title, type, firstPrize, secondPrize, thirdPrize, $id,startDate,endDate }:TournamentCreationParams) {
    try {
  ...
  async quizFileUpload(file:any) {
    try {
      return await this.bucket.createFile(`${process.env.NEXT_PUBLIC_BUCKET_ID}`, ID.unique(), file)
    } catch (error) {
  ...
  async quizImagePreview(imageId:any) {
    try {
      return await this.bucket.getFilePreview(`${process.env.NEXT_PUBLIC_BUCKET_ID}`, imageId)
    } catch (error) {
  ...
  async createQuiz({
   ...
   async getAllQuestionsByTournamentId(selectedTournamentId: string | number | boolean | QueryTypesList) {
    try {
      const response = await this.databases.listDocuments(
        `${process.env.NEXT_PUBLIC_DATABASEID}`,
  ...
  async deleteQuiz(id: string) {
    ...
    async deleteQuizImage(imageId: string) {
      try {
        if (imageId) {
          await this.bucket.deleteFile(`${process.env.NEXT_PUBLIC_QUIZ_BUCKET_ID}`, imageId)
          return true
        }
        return
      } catch (error) {
...
async getSingleQuizById(id:string){
  ...
  async createQuestions({  options, type, question, selectedTournamentId, correctAnswer, imageUrl, imageId }:CreateQuestionsParams) {
    // console.log( emoji)
    try {
      const document = await this.databases.createDocument(
        `${process.env.NEXT_PUBLIC_DATABASEID}`,
